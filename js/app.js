...
function updateAuthUI(user) {
    if (user) {
        ...
        // Limpiar tareas locales si no hay usuario
        appState.tasks = [];
        renderTasks();
        renderBubbles();
    }
...
async function saveTaskToFirestore(task) {
    if (!appState.currentUser) {
        // Si no hay usuario, guardar solo localmente
        console.log('Guardando tarea localmente (sin usuario)');
        return task.id;
    }
    ...
    try {
        ...
        return task.id; // Fallback a ID local
    }
}
...
function renderTasks() {
    ...
    if (appState.tasks.length === 0) {
        elements.taskList.innerHTML = '<p class="no-tasks">No hay tareas aún. ¡Agrega una!</p>';
        return;
    }
...
// ---- 8. LÓGICA DE TAREAS Y SELECCIÓN ----
